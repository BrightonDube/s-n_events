...
interface HoverButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  /**
   * Legacy props kept solely for backward-compatibility during the Tailwind v4 upgrade.
   * They have **no effect** on rendering and will be removed in a future cleanup pass.
   */
  variant?: string
  size?: string
  asChild?: boolean
...
const HoverButton = React.forwardRef<HTMLButtonElement, HoverButtonProps>(
  (
    { className, children, variant: _variant, size: _size, asChild: _asChild, ...props },
    ref
  ) => {
    ...
    return (
      <button
        ref={buttonRef}
        className={cn(
          "relative isolate px-8 h-12 flex flex-nowrap items-center justify-center gap-2 rounded-3xl",
          "text-foreground font-medium text-base leading-6",
          "backdrop-blur-lg bg-[rgba(43,55,80,0.1)]",
          "cursor-pointer overflow-hidden",
          "hover:bg-brand-accent hover:text-white transition-colors duration-300",
        ...
        {...props}
        style={{
          "--circle-start": "var(--color-brand-primary, #ED85A7)",
          "--circle-end": "var(--color-brand-accent, #3F232C)",
